...
def read_original_video_pyav(container, intervals, num_frames):
    ...
    Args:
        ...
        num_frames (int): The fixed number of maximum number of frames to extract (default 150).

    Returns:
        result (np.ndarray): np array of decoded frames of shape (frames, height, width, 3).
    """
    frames = []
    container.seek(0)
...
def run(video_folder, video_prompt, quant=True):
    ...
    # Uncomment this pair (and comment the other) to evaluate subset with at least 3 top scoring fragments
    summe_excluded = {1, 3, 4, 5, 6, 7, 8, 9, 11, 13, 14, 15, 16, 17, 18, 19, 20, 23, 24, 25}   # exlude SumMe videos that do NOT have at least 3 top scoring fragments
    tvsum_excluded = {1, 3, 6, 12, 15, 21, 29, 37, 39, 41, 46}                                  # exlude TVSum videos that do NOT have at least 3 top scoring fragments
    ...
    for video_subfolder in video_subfolders:
        ...
        summary_top_1_fragment = [video_sum_indices[0]]
        summary_top_2_fragment = [video_sum_indices[1]]
        summary_top_3_fragment = [video_sum_indices[2]]
        ...
        summary_top_1_video = read_video_pyav(container, summary_top_1_fragment, downsample_rate=15, num_frames=150)
        summary_top_2_video = read_video_pyav(container, summary_top_2_fragment, downsample_rate=15, num_frames=150)
        summary_top_3_video = read_video_pyav(container, summary_top_3_fragment, downsample_rate=15, num_frames=150)
        ...
        #       f"Summary Top 1: {summary_top_1_video.shape}\nSummary Top 2: {summary_top_2_video.shape}\nSummary Top 3: {summary_top_3_video.shape}\n"
        ...
        if any(v is None for v in [video, video_sum, video_expl_attention, video_expl_lime, summary_top_1_video, summary_top_2_video, summary_top_3_video,
        ...
        summary_top_1 = process_video_fragment(model, processor, device, summary_top_1_video, video_prompt, video_prompt_length)
        summary_top_2 = process_video_fragment(model, processor, device, summary_top_2_video, video_prompt, video_prompt_length)
        summary_top_3 = process_video_fragment(model, processor, device, summary_top_3_video, video_prompt, video_prompt_length)
        ...
        alternative_summary = create_alternative_sequential_explanation(model, processor, device, summary_top_1[0], summary_top_2[0], summary_top_3[0], prompt_2, video_sum_indices)
        ...
        sentences.append(original_video[0])                     #0
        sentences.append(video_summary[0])                      #1
        sentences.append(alternative_summary[0])                #2
        sentences.append(attention_explanation[0])              #3
        sentences.append(attention_alternative_explanation[0])  #4
        sentences.append(lime_explanation[0])                   #5
        sentences.append(lime_alternative_explanation[0])       #6
        sentences.append(attention_top_1[0])                    #7
        sentences.append(attention_top_2[0])                    #8
        sentences.append(attention_top_3[0])                    #9
        sentences.append(attention_top_1_2[0])                  #10
        sentences.append(lime_top_1[0])                         #11
        sentences.append(lime_top_2[0])                         #12
        sentences.append(lime_top_3[0])                         #13
        sentences.append(lime_top_1_2[0])                       #14
        ...
        with open(video_result_path, 'a') as file:
            ...
            file.write('alternative_video_summary:\n')
            file.write('\n'.join(alternative_summary[1]) + '\n\n')
        ...
        simcse_alternative_attention_sum = compute_simcse_similarity(get_embeddings(video_summary[0]), get_embeddings(attention_alternative_explanation[0]))
        simcse_alternative_lime_sum = compute_simcse_similarity(get_embeddings(video_summary[0]), get_embeddings(lime_alternative_explanation[0]))
        simcse_attention_alternative_sum = compute_simcse_similarity(get_embeddings(alternative_summary[0]), get_embeddings(attention_explanation[0]))
        simcse_lime_alternative_sum = compute_simcse_similarity(get_embeddings(alternative_summary[0]), get_embeddings(lime_explanation[0]))
        simcse_alternative_attention_alternative_sum = compute_simcse_similarity(get_embeddings(alternative_summary[0]), get_embeddings(attention_alternative_explanation[0]))
        simcse_alternative_lime_alternative_sum = compute_simcse_similarity(get_embeddings(alternative_summary[0]), get_embeddings(lime_alternative_explanation[0]))
        ...
        similarities_results.append([simcse_attention, simcse_lime, simcse_attention_sum, simcse_lime_sum,
                                     simcse_alternative_attention_sum, simcse_alternative_lime_sum,
                                     simcse_attention_alternative_sum, simcse_lime_alternative_sum,
                                     simcse_alternative_attention_alternative_sum, simcse_alternative_lime_alternative_sum,
                                     ...
                                     round(sbert_similarities[0][3].item(), 5), round(sbert_similarities[0][5].item(), 5),
                                     round(sbert_similarities[1][3].item(), 5), round(sbert_similarities[1][5].item(), 5),
                                     round(sbert_similarities[1][4].item(), 5), round(sbert_similarities[1][6].item(), 5),
                                     round(sbert_similarities[2][3].item(), 5), round(sbert_similarities[2][5].item(), 5),
                                     round(sbert_similarities[2][4].item(), 5), round(sbert_similarities[2][6].item(), 5),
                                     round(sbert_similarities[0][7].item(), 5), round(sbert_similarities[0][11].item(), 5),
                                     round(sbert_similarities[0][8].item(), 5), round(sbert_similarities[0][12].item(), 5),
                                     round(sbert_similarities[0][9].item(), 5), round(sbert_similarities[0][13].item(), 5),
                                     round(sbert_similarities[0][10].item(), 5), round(sbert_similarities[0][14].item(), 5),
                                     round(sbert_similarities[1][7].item(), 5), round(sbert_similarities[1][11].item(), 5),
                                     round(sbert_similarities[1][8].item(), 5), round(sbert_similarities[1][12].item(), 5),
                                     round(sbert_similarities[1][9].item(), 5), round(sbert_similarities[1][13].item(), 5),
                                     round(sbert_similarities[1][10].item(), 5), round(sbert_similarities[1][14].item(), 5)])
        ...
        similarity_columns = [
            ...
            'simcse_attention_top_1_sum', 'simcse_lime_top_1_sum', 'simcse_attention_top_2_sum', 'simcse_lime_top_2_sum',
            'simcse_attention_top_3_sum', 'simcse_lime_top_3_sum', 'simcse_attention_top_1_2_sum', 'simcse_lime_top_1_2_sum',
            'sbert_attention', 'sbert_lime', 'sbert_attention_sum', 'sbert_lime_sum', 'sbert_alternative_attention_sum', 'sbert_alternative_lime_sum',
            'sbert_attention_alternative_sum', 'sbert_lime_alternative_sum',
            'sbert_alternative_attention_alternative_sum', 'sbert_alternative_lime_alternative_sum',
...
if __name__ == "__main__":
    ...
    while True:
        try:
            run(video_folder=data_folder, video_prompt=prompt)
            break
