...
videos = [
    [v for v in all_video_indices["SumMe"] if v not in videosets[videoset_key]["SumMe"]],
    [v for v in all_video_indices["TVSum"] if v not in videosets[videoset_key]["TVSum"]]
]

# Compute the fragment-level evaluation scores
def fragments_explanation_scores(video_path):
    ...
    if not np.isnan(video_scores[0][0]) and not np.isnan(video_scores[0][2]):
        temp = np.copy(video_scores[:1])
        temp[:, 0:4] = np.arctanh(temp[:, 0:4])
        fragment_scores_1.append(temp)

    if not np.isnan(video_scores[1][0]) and not np.isnan(video_scores[1][2]):
        temp = np.copy(video_scores[:2])
        temp[:, 0:4] = np.arctanh(temp[:, 0:4])
        fragment_scores_2.append(temp)

    if not np.isnan(video_scores[2][0]) and not np.isnan(video_scores[2][2]):
...
for d in range(len(dataset)):
    ...
    for vid in videos[d]:
        video_path = f"../data/{dataset[d]}/video_{vid}/explanation/"
    ...
    columns_names = [
        "Attention Disc Plus One By One", "Attention Disc Plus Sequentially",
        "Lime Disc Plus One By One", "Lime Disc Plus Sequentially"
    ]
